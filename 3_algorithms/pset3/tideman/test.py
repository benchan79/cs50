...
def exists():
    """tide.c exists"""
    check50.exists("tide.c")
    check50.include("test.c")

@check50.check(exists)

def compiles():
    """tideman compiles"""
    check50.c.compile("tide.c", lcs50=True)
    tideman = re.sub("int\s+main", "int distro_main", open("tide.c").read())
    testing = open("test.c").read()
